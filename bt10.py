...
X_train, y_train = train_boost10()
print(X_train.head())
mod_boost10 = GradientBoostingRegressor(max_depth=11)
mod_boost10.fit(X_train, y_train.values.ravel())
print(mod_boost10.feature_importances_)
y_train_pred = mod_boost10.predict(X_train)
print(min(y_train_pred))
y_train_pred = np.maximum(0,y_train_pred)
print(np.sqrt(mean_squared_log_error(y_train,y_train_pred)))

with open("tree10.pkl", "wb") as f:
     pickle.dump(mod_boost10, f)
