...
X_train, y_train = train_boost9()
print(X_train.head())
mod_boost9 = GradientBoostingRegressor(max_depth=12)
mod_boost9.fit(X_train, y_train.values.ravel())
print(mod_boost9.feature_importances_)
y_train_pred = mod_boost9.predict(X_train)
print(min(y_train_pred))
y_train_pred = np.maximum(0,y_train_pred)
print(np.sqrt(mean_squared_log_error(y_train,y_train_pred)))

with open("tree9.pkl", "wb") as f:
     pickle.dump(mod_boost9, f)
