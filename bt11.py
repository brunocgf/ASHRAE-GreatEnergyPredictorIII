...
X_train, y_train = train_boost11()
print(X_train.head())
mod_boost11 = GradientBoostingRegressor(max_depth=12)
mod_boost11.fit(X_train, y_train.values.ravel())
print(mod_boost11.feature_importances_)
y_train_pred = mod_boost11.predict(X_train)
print(min(y_train_pred))
y_train_pred = np.maximum(0,y_train_pred)
print(np.sqrt(mean_squared_log_error(y_train,y_train_pred)))

with open("tree11.pkl", "wb") as f:
     pickle.dump(mod_boost11, f)
