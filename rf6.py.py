...
X_train, y_train = train_rf6()
print(X_train.head())
mod_rf6 = RandomForestRegressor(max_depth=12)
mod_rf6.fit(X_train, y_train.values.ravel())
print(mod_rf6.feature_importances_)
y_train_pred = mod_rf6.predict(X_train)
print(min(y_train_pred))
y_train_pred = np.maximum(0,y_train_pred)
print(np.sqrt(mean_squared_log_error(y_train,y_train_pred)))

with open("rf6.pkl", "wb") as f:
     pickle.dump(mod_rf6, f)


#with open("tree4rev.pkl","rb") as f:
#    mod_rf4 = pickle.load(f)


X_test = test_rf6()
y_pred = mod_rf6.predict(X_test)
sub = pd.DataFrame(np.maximum(0,y_pred), index = X_test.index, columns = ['meter_reading'])
sub.sort_values(by = 'row_id', inplace = True)
sub.to_csv('./submissionrf6.csv')